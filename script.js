$(function() { // Makes sure that your function is called once all the DOM elements of the page are ready to be used.
    ...
    // Live expression demo
    let debugCounter = 0;
    setInterval(() => {debugCounter++; }, 1000);
    
    // Console utilities demo
    console.table(pet_info);
    
    // Optimal fetch demo (for XHR/fetch breakpoints)
    async function fetchPetData() {
        const res = await fetch('assets/pet-data.json');
        const data = await res.json();
        console.log("Fetched pet data:", data);
    }
    // Attach fetch to exercise button for demo
    $('.exercise-button').click(fetchPetData);
